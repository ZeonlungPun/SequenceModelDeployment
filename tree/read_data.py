...
def get_train_data(dataset_path,train_path):
    ...
    x_train_signals_paths=[os.path.join(dataset_path,train_path,'InertialSignals',signal+'train.txt') for signal in inertial_signals]
    xtrain=get_x_data(x_train_signals_paths)
    y_train_path=os.path.join(dataset_path,train_path,'y_train.txt')
...
def get_test_data(dataset_path,test_path):
    inertial_signals=['body_acc_x_','body_acc_y_','body_acc_z_','body_gyro_x_','body_gyro_y_','body_gyro_z_','total_acc_x_','total_acc_y_','total_acc_z_']
    x_test_signals_paths=[os.path.join(dataset_path,test_path,'InertialSignals',signal+'test.txt') for signal in inertial_signals]
    xtest=get_x_data(x_test_signals_paths,data_num=2947)
    y_test_path=os.path.join(dataset_path,test_path,'y_test.txt')
